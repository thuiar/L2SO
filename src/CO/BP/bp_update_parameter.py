...
try:
    ...
    print("Warning: requests library not found. LLM communication relies on it.")
    requests = None
...
# Handle AST manipulation dependencies (astor for Python < 3.9, ast.unparse for >= 3.9)
try:
    import astor
except ImportError:
    if sys.version_info < (3, 9):
        print("Warning: 'astor' library not found. AST manipulation requires astor for Python < 3.9. Functionality may be limited.")
    astor = None
...
def ast_to_source(tree):
    if hasattr(ast, 'unparse'):
        ...
        raise RuntimeError("Cannot convert AST to source. Neither ast.unparse (Python >= 3.9) nor astor is available.")


# Assume selection and management are local libraries as specified.
# We must not modify their usage.
try:
    from selection import prob_rank,equal,roulette_wheel,tournament
    from management import pop_greedy,ls_greedy,ls_sa
except ImportError:
    # Provide placeholders if libraries are missing to allow script loading, but execution will fail as expected.
    print("Warning: Local libraries 'selection' or 'management' not found. Execution will fail if these methods are called.")
...
class Paras():
    def __init__(self):
        ...
        self.problem = 'bp_online'     # Selected problem
        ...
        self.ec_m = 2  # number of parents for crossover operators, default = 2
        ...
        self.opt_range_factor = 0.01 # Factor to define search range around default values
        self.opt_storage_path = "sqlite:///eoh_optimization.db" # Storage for Optuna studies


    def set_parallel(self):
        #################################
        ###  Set the number of processes to the machine maximum ###
        #################################
        try:
    ...
    def set_ec(self):    
        ###########################################################
        ###  Set population management strategy, parent selection strategy, and evolution strategies/weights
        ###########################################################
    ...
    def set_evaluation(self):
        #################################
        ###  Set evaluation parameters (based on the problem)
        #################################
    ...
    def set_paras(self, *args, **kwargs):
        #################################
        ###  Set parameters and initialize settings
        #################################
...
def create_folders(results_path):
    #####################################################
    ###  Create results folder and subfolders (history, pops, pops_best)
    #####################################################
...
############################################################################################################
###  AST Utility Functions (Merged and standardized)
############################################################################################################
...
def add_import_package_statement(program: str, package_name: str, as_name=None, *, check_imported=True) -> str:
    ...
    # Insert the new import node at the top of the AST
    tree.body.insert(0, import_node)
    ast.fix_missing_locations(tree)
    # Convert the modified AST back to Python code string
...
def _add_numba_decorator(
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.FunctionDef) and node.name == function_name:
            # Check if decorator already exists (improved detection)
            already_decorated = False
            for decorator in node.decorator_list:
                # Check for @jit, @njit, @numba.jit, @numba.njit or any similar decorator
                if isinstance(decorator, ast.Call):
                    func = decorator.func
                    if isinstance(func, ast.Attribute) and func.attr in ['jit', 'njit']:
                        already_decorated = True
                        break
                    elif isinstance(func, ast.Name) and func.id in ['jit', 'njit']:
                        already_decorated = True
                        break
                elif isinstance(decorator, ast.Attribute) and decorator.attr in ['jit', 'njit']:
                    already_decorated = True
                    break
                elif isinstance(decorator, ast.Name) and decorator.id in ['jit', 'njit']:
                    already_decorated = True
                    break
            
            if already_decorated:
                # Function already has a JIT decorator, return original program
                return program
...
def remove_numba_decorator(code_string: str) -> str:
    """
    Remove all Numba decorators from code string using regex.
    """
    # Remove @numba.jit, @jit, @njit, or @numba.njit decorators from the code string
    import re
    code_string = re.sub(r'@(?:numba\.)?(?:jit|njit)\([^)]*\)\s*\n', '', code_string)
    code_string = re.sub(r'@(?:numba\.)?(?:jit|njit)\s*\n', '', code_string)
    return code_string


def fix_numba_incompatible_code(code_string: str) -> str:
    """
    Fix common Numba incompatibilities in generated code.
    """
    # Replace np.zeros_like with simple numpy array creation
    code_string = re.sub(
        r'np\.zeros_like\s*\(\s*([^,)]+)\s*,\s*dtype\s*=\s*float\s*\)', 
        r'np.zeros(\1.shape)', 
        code_string
    )
    code_string = re.sub(
        r'np\.zeros_like\s*\(\s*([^,)]+)\s*\)', 
        r'np.zeros(\1.shape)', 
        code_string
    )
    
    # Replace np.ones_like with simple numpy array creation
    code_string = re.sub(
        r'np\.ones_like\s*\(\s*([^,)]+)\s*,\s*dtype\s*=\s*float\s*\)', 
        r'np.ones(\1.shape)', 
        code_string
    )
    code_string = re.sub(
        r'np\.ones_like\s*\(\s*([^,)]+)\s*\)', 
        r'np.ones(\1.shape)', 
        code_string
    )
    
    # Fix the specific problematic pattern from the log:
    # The issue is that LLM creates scores array with same shape as bins, but should return scores only for feasible bins
    # This is a fundamental logic error in the generated code
    
    # Pattern 1: scores = np.zeros_like(bins) or np.zeros(bins.shape) with later assignment
    pattern1 = r'scores = np\.zeros(?:_like)?\([^)]*bins[^)]*\).*?\n.*?if feasible_bins\.size > 0:\s*\n\s*scores\[bins > item\]\s*=\s*([^\n]+)'
    
    def fix_scores_logic(match):
        score_calculation = match.group(1)
        return f'''if feasible_bins.size > 0:
        scores = {score_calculation}
    else:
        scores = np.array([])'''
    
    code_string = re.sub(pattern1, fix_scores_logic, code_string, flags=re.MULTILINE | re.DOTALL)
    
    # Pattern 2: Direct replacement of problematic zeros_like patterns
    if 'np.zeros_like(bins' in code_string and 'scores[bins > item]' in code_string:
        # This is the exact problematic pattern, replace the entire logic
        ...
        for line in lines:
            ...
            elif inside_function and line.strip().startswith('scores = np.zeros'):
                # Skip this line and replace with proper logic
                continue
            elif inside_function and 'scores[bins > item]' in line:
                # Replace the assignment with proper return logic
                indent = len(line) - len(line.lstrip())
                score_calc = line.split('=')[1].strip()
                new_lines.append(' ' * indent + f'scores = {score_calc}')
                new_lines.append(' ' * (indent-4) + 'else:')
                new_lines.append(' ' * indent + 'scores = np.array([])')
            elif inside_function and line.strip() == 'return scores':
                new_lines.append(line)
                inside_function = False
            else:
                new_lines.append(line)
...
#######################################
#######################################
###  Problem Definition (Bin Packing Online)
#######################################
#######################################

class GetPrompts():
    #####################################################
    ###  Prompt class for Online Bin Packing
    #####################################################
    def __init__(self):
...
The output named 'scores' is the scores for the bins for assignment. "
        # Clarified type (int or float) based on common BP implementations
        self.prompt_other_inf = "Note that 'item' is of type int (or float), while 'bins' and 'scores' are both Numpy arrays. The function will be optimized with Numba JIT compilation, so avoid using np.zeros_like, np.ones_like, or dtype parameters. Use simple NumPy operations compatible with Numba nopython mode. The 'scores' array should have the same length as the input 'bins' array. The novel function should be sufficiently complex in order to achieve better performance. It is important to ensure self-consistency."
...
class GetData():
    # !!! CRITICAL: DO NOT MODIFY THIS CLASS AS PER USER INSTRUCTIONS !!!
    ...
    def l1_bound_dataset(self,instances: dict) -> float:
        ...
        return np.mean(l1_bounds) if l1_bounds else 0

    def get_instances(self):
        opt_num_bins = {}
        for name, dataset in self.datasets.items():
            opt_num_bins[name] = self.l1_bound_dataset(dataset)
        print("L1 Lower Bounds:", opt_num_bins)
        return self.datasets, opt_num_bins

    
class PROBLEMCONST():
    ###########################################
    ###  Problem definition and evaluation for Online Bin Packing
    ###########################################
    def __init__(self):
        getdate = GetData()
        ...
        self.use_numba = True   # Enable Numba JIT compilation for performance

    def get_valid_bin_indices(self,item: float, bins: np.ndarray) -> np.ndarray:
        """Returns indices of bins in which item can fit."""
        # Use a small tolerance for float comparison
        return np.nonzero((bins - item) >= -1e-9)[0]


    # ADAPTED: Now accepts optional 'params' for parameter optimization.
    def online_binpack(self, items: tuple, bins: np.ndarray, alg, params=None):
        """Performs online binpacking of `items` into `bins`."""
        # Track which items are added to each bin.
        packing = [[] for _ in bins]
        
        if self.debug_mode and params is not None:
            print(f"DEBUG: online_binpack called with params: {params}")
        
        # Simplified parameter checking to avoid memory conflicts in parallel execution
        accepts_params = False
        if params is not None:
            # Try calling with params, fall back if it fails
            try:
                # Test call to see if function accepts params
                test_bins = np.array([1.0])  # Simple test array
                test_item = 0.5
                valid_indices = test_bins > test_item
                if np.any(valid_indices):
                    if self.debug_mode:
                        print(f"DEBUG: Testing params support with test_item={test_item}, test_bins={test_bins[valid_indices]}, params={params}")
                    # Try with the actual params to ensure they work
                    result = alg.score(test_item, test_bins[valid_indices], params)
                    if self.debug_mode:
                        print(f"DEBUG: Parameter test successful, result={result}")
                    accepts_params = True
            except (TypeError, Exception) as e:
                if self.debug_mode:
                    print(f"DEBUG: Parameter test failed: {type(e).__name__}: {e}")
                    import traceback
                    traceback.print_exc()
                accepts_params = False

        if self.debug_mode:
            print(f"DEBUG: accepts_params = {accepts_params}")
        ...
        for i, item in enumerate(items):
            ...
            if len(valid_bin_indices) == 0:
                if self.debug_mode:
                    print(f"DEBUG: No valid bins for item {item} at position {i}")
                # Should not happen if enough bins are initialized
                continue

            # Score each bin based on heuristic.
            # Simplified call to avoid memory conflicts
            try:
                if accepts_params and params is not None:
                    if self.debug_mode and i < 2:  # Debug first 2 items only
                        print(f"DEBUG: Calling score with params for item {item}")
                    priorities = alg.score(item, bins[valid_bin_indices], params)
                elif accepts_params:
                    if self.debug_mode and i < 2:
                        print(f"DEBUG: Calling score with empty params for item {item}")
                    # If function accepts params but none provided, use empty dict
                    priorities = alg.score(item, bins[valid_bin_indices], {})
                else:
                    if self.debug_mode and i < 2:
                        print(f"DEBUG: Calling score without params for item {item}")
                    # Standard call (non-parameterized code)
                    priorities = alg.score(item, bins[valid_bin_indices])
                    
                if self.debug_mode and i < 2:
                    print(f"DEBUG: Priorities for item {item}: {priorities}")
                    
            except Exception as e:
                if self.debug_mode:
                    print(f"DEBUG: Score function failed for item {item}: {type(e).__name__}: {e}")
                    import traceback
                    traceback.print_exc()
                # Fallback to simple call if parameter call fails
                try:
                    priorities = alg.score(item, bins[valid_bin_indices])
                    if self.debug_mode:
                        print(f"DEBUG: Fallback score successful for item {item}")
                except Exception as e2:
                    if self.debug_mode:
                        print(f"DEBUG: Fallback score also failed for item {item}: {e2}")
                    return None, None
            
            if priorities is None or len(priorities) == 0:
                if self.debug_mode:
                    print(f"DEBUG: Score function returned None/empty for item {item}")
                return None, None
    ...
    # ADAPTED: Now accepts optional 'params' for parameter optimization.
    def evaluateGreedy(self,alg, params=None) -> float:
        """Evaluate heuristic function on a set of online binpacking instances."""

        if self.debug_mode and params is not None:
            print(f"DEBUG: evaluateGreedy called with params: {params}")

        # Initialize fitness tracking
        total_fitness = 0
        dataset_count = 0

        # Iterate over datasets (e.g., 'Weibull 5k')
        for name, dataset in self.instances.items():
            if self.debug_mode:
                print(f"DEBUG: Processing dataset: {name}")
            num_bins_list = []
            # Iterate over instances within the dataset
            for instance_id, instance in dataset.items():
                if self.debug_mode and len(num_bins_list) == 0:  # Debug first instance only
                    print(f"DEBUG: Processing instance {instance_id} in dataset {name}")
                ...
                # Initialize bins (Use float type for capacities)
                bins = np.array([capacity for _ in range(num_items)], dtype=float)
                
                # Pack items into bins. Pass 'params' argument down.
                try:
                    if self.debug_mode and len(num_bins_list) == 0:
                        print(f"DEBUG: Calling online_binpack for instance {instance_id} with {num_items} items")
                    
                    packing, bins_packed = self.online_binpack(items, bins, alg, params)
                    
                    if packing is None or bins_packed is None:
                        if self.debug_mode:
                            print(f"DEBUG: online_binpack returned None for instance {instance_id}")
                        return None # Indicate failure
                        
                    if self.debug_mode and len(num_bins_list) == 0:
                        print(f"DEBUG: online_binpack completed successfully for instance {instance_id}")
                        
                except Exception as e:
                    if self.debug_mode:
                        print(f"DEBUG: Exception in online_binpack for instance {instance_id}: {type(e).__name__}: {e}")
                        import traceback
                        traceback.print_exc()
                    return None # Indicate failure

                # Count number of used bins (Use tolerance for float comparison)
                num_bins = (bins_packed < capacity - 1e-9).sum()
                
                if self.debug_mode and len(num_bins_list) == 0:
                    print(f"DEBUG: Instance {instance_id} used {num_bins} bins")

                num_bins_list.append(num_bins)

            if not num_bins_list:
                if self.debug_mode:
                    print(f"DEBUG: No valid instances processed for dataset {name}")
                continue

            # Calculate average number of bins used for this dataset
            avg_num_bins = np.mean(np.array(num_bins_list))
            
            # Calculate fitness as the relative gap from the lower bound (L1 bound)
            # Minimization objective.
            lower_bound = self.lb.get(name, 0)
            if lower_bound > 0:
                fitness = (avg_num_bins - lower_bound) / lower_bound
            else:
                # Handle edge case where lower bound is 0 or missing
                fitness = avg_num_bins

            if self.debug_mode:
                print(f"DEBUG: Dataset {name} - avg_bins: {avg_num_bins:.3f}, lower_bound: {lower_bound}, fitness: {fitness:.6f}")

            total_fitness += fitness
            dataset_count += 1

        # Return average fitness across all datasets
        final_fitness = total_fitness / dataset_count if dataset_count > 0 else None
        if self.debug_mode:
            print(f"DEBUG: Final fitness: {final_fitness}")
        return final_fitness

    # ADAPTED: Now accepts optional 'params' and 'n_evals' for robust evaluation.
    def evaluate(self, code_string, params=None, n_evals=1):
        try:
            # Force garbage collection at start to free memory
            import gc
            gc.collect()
            
            if self.debug_mode:
                print(f"DEBUG: Starting evaluation with n_evals={n_evals}")
                print(f"DEBUG: Code string length: {len(code_string)}")
                
            # Keep Numba decorators for better performance
            if self.debug_mode and params is not None:
                print("DEBUG: Using Numba-compatible parameter optimization")
            ...
            with warnings.catch_warnings():
                ...
                # Ensure numpy is available in the module's namespace
                heuristic_module.__dict__['np'] = np
                
                # Ensure numba is available in the module's namespace (if using numba)
                if self.use_numba:
                    import numba
                    heuristic_module.__dict__['numba'] = numba
                
                if self.debug_mode:
                    print("DEBUG: Executing code string...")
                    print("DEBUG: Code string content:")
                    print(code_string)
                    print("DEBUG: End of code string")
                ...
                if self.debug_mode:
                    print(f"DEBUG: Module created, has score function: {hasattr(heuristic_module, 'score')}")
                    if hasattr(heuristic_module, 'score'):
                        score_func = heuristic_module.score
                        print(f"DEBUG: Score function: {score_func}")
                        # Test the score function signature
                        import inspect
                        try:
                            sig = inspect.signature(score_func)
                            print(f"DEBUG: Score function signature: {sig}")
                            print(f"DEBUG: Score function parameters: {list(sig.parameters.keys())}")
                        except Exception as e:
                            print(f"DEBUG: Could not inspect signature: {e}")
                    else:
                        print("DEBUG: No score function found in module!")
                        return None
                ...
                for eval_round in range(n_evals):
                    if self.debug_mode:
                        print(f"DEBUG: Evaluation round {eval_round + 1}/{n_evals}")
                    
                    # Pass 'params' to the evaluation function
                    fitness = self.evaluateGreedy(heuristic_module, params)
                    
                    if self.debug_mode:
                        print(f"DEBUG: Fitness from evaluateGreedy: {fitness}")
                    
                    if fitness is not None:
                        fitness_values.append(fitness)

                if self.debug_mode:
                    print(f"DEBUG: Total valid fitness values: {len(fitness_values)}")

                if not fitness_values:
                    if self.debug_mode:
                        print("DEBUG: No valid fitness values, returning None")
                ...
                if self.debug_mode:
                    print(f"DEBUG: Median fitness: {median_fitness}")

                # Clean up to prevent memory leaks in parallel execution
                try:
                    if heuristic_module.__name__ in sys.modules:
                        del sys.modules[heuristic_module.__name__]
                except Exception:
                    pass
...
class Probs():
    ###########################################
    ###  Load or create problem instances using PROBLEMCONST()
    ###########################################
    def __init__(self,paras):
        if not isinstance(paras.problem, str):
            ...
            # Create new Bin Packing problem instance
            self.prob = PROBLEMCONST()
            print("- Prob "+paras.problem+" loaded ")
        elif paras.problem == "tsp_construct":
             # The provided PROBLEMCONST handles bp_online
            self.prob = PROBLEMCONST()
            print("- Prob "+paras.problem+" loaded (using BP evaluator structure)")
        else:
...
class InterfaceAPI:
    #######################################
    ###  Call API to communicate with LLM
    #######################################
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        self.api_endpoint = api_endpoint # API endpoint address
        self.api_key = api_key           # API key
        self.model_LLM = model_LLM       # LLM model name
        self.debug_mode = debug_mode     # Debug mode flag
        self.n_trial = 5                 # Max attempts

    def get_response(self, prompt_content):
        if requests is None:
            print("Error: requests library missing. Cannot communicate with LLM.")
            return None
        ...
        while n_trial < self.n_trial:
            n_trial += 1
            try:
                # Use requests library for robust HTTP handling
                ...
                if self.debug_mode:
                    print(f"Error in API (Attempt {n_trial}/{self.n_trial}): {e}. Retrying...")
                time.sleep(2) # Exponential backoff might be better, but simple delay is sufficient
                continue
...
class InterfaceLLM:
    ...
    def get_response(self, prompt_content):
        ##############################################################################
        # Get LLM response for given content.
        ##############################################################################
...
############################################################################################################
###  PARAMETERSEARCH Class (Integrated and Adapted for Bin Packing)
############################################################################################################

class PARAMETERSEARCH():
    ###########################################
    ###  Class for automatic parameter optimization using LLM and Optuna
    ###########################################
    def __init__(self, interface_eval, interface_llm, paras):
        if optuna is None:
            # print("Parameter optimization initialized but Optuna is missing. Optimization will be skipped.")
            self.enabled = False
            return
        
        self.enabled = True
        
        # Initialize LLM interface (passed from EOH runner)
        self.interface_llm = interface_llm
        if self.interface_llm is None:
            # print("LLM Interface not available. Parameter optimization disabled.")
            self.enabled = False
            return

        # Set the evaluator (PROBLEMCONST instance for Bin Packing)
        ...
        # Define the target function name for Bin Packing
        self.target_func_name = "score"
        
        # Add code recording capability
        self.code_history_path = None  # Will be set by the caller
    
    def set_code_history_path(self, path):
        """Set the path for saving code history."""
        self.code_history_path = path
    
    def save_code_with_removed_numba(self, code_string, filename):
        """
        Save code with numba decorators removed to a file.
        """
        # Remove numba decorators
        cleaned_code = remove_numba_decorator(code_string)
    ...
    def construct_prompt_with_emphasis_on_format(self, code_string):
        """
        Constructs the prompt for the LLM to extract parameters and modify the code.
        ADAPTED: Example reflects the Bin Packing 'score' function.
        """
        prompt = (
            ...
            "    # Function logic...\n"
            "```\n\n"
            "--- Example (Online Bin Packing Heuristic) ---\n"
            "Input Code:\n"
            "```python\n"
            "import numpy as np\n"
            "def score(item, bins):\n"
            "    remaining_capacity = bins\n"
            "    fitness_weight = 0.7\n"
            "    scores = fitness_weight * (1 - remaining_capacity/100.0) + 0.3 * np.exp(-item/10.0)\n"
            "    return scores\n"
            "```\n\n"
            "Expected Output:\n"
            "1. Extracted Parameters:\n"
            ...
            "{\n"
            "    \"fitness_weight\": 0.7,\n"
            "    \"item_weight_factor\": 0.3,\n"
            "    \"item_exp_divisor\": 10.0\n"
            "}\n"
            "```\n\n"
            "2. Modified Code:\n"
            "```python\n"
            ...
            "from numba import jit\n\n"
            "@jit(nopython=True)\n"
            "def score(item, bins, params):\n"
            "    # Extract parameters using .get() method\n"
            "    fitness_weight = params.get(\"fitness_weight\", 0.7)\n"
            "    item_weight_factor = params.get(\"item_weight_factor\", 0.3)\n"
            "    item_exp_divisor = params.get(\"item_exp_divisor\", 10.0)\n"
            "    remaining_capacity = bins\n"
            "    # Note: Assuming capacity is 100.0 if hardcoded in original logic.\n"
            "    scores = fitness_weight * (1 - remaining_capacity/100.0) + item_weight_factor * np.exp(-item/item_exp_divisor)\n"
            "    return scores\n"
            "```\n"
            "--- End of Example ---\n\n"
            "IMPORTANT: Use params.get() method for parameter extraction as shown in the example.\n"
            "Also ensure ALL extracted parameters are actually used in the algorithm logic!\n\n"
    ...
    def parse_llm_response(self, response):
        try:
            if self.debug_mode:
                print("--- LLM Response for Parameter Extraction ---\n", response, "\n-------------------------------------------")
            ...
            if code_match:
                ...
                # If code is missing but parameters were found, it's an error in LLM formatting
                if parameters:
                    raise ValueError("Modified code block not found, but parameters were extracted.")
                else:
                    # If both are missing, assume no modification needed.
                    return {}, None
            
            # Ensure numpy and numba are imported, and add numba decorator
            modified_code = add_import_package_statement(modified_code, 'numpy', 'np')
            modified_code = add_import_package_statement(modified_code, 'numba')
            
            # Add numba decorator to the score function if not present
            if 'def score(' in modified_code and '@jit' not in modified_code:
                modified_code = add_numba_decorator(modified_code, 'score')
    ...
    def extract_parameters_with_llm(self, code_string):
        try:
            # Construct prompt
    ...
    def insert_parameters_into_code(self, modified_code, best_params, target_func_name):
        """
        Inserts the optimized parameters back into the code by replacing dynamic retrieval 
        with hardcoded values using AST manipulation. This version is enhanced to handle 
        inline `params.get()` calls within expressions.
        """
        if self.debug_mode:
            print("Original parameterized code:\n", modified_code)
            print("Best parameters to insert:", best_params)
        try:
            # Parse the code string into AST
            tree = ast.parse(modified_code)

            # AST Transformer to insert parameters and handle inline calls
            class ParameterInserter(ast.NodeTransformer):
                def visit_FunctionDef(self, node):
                    # Find the target function (e.g., 'score')
                    if node.name == target_func_name:
                        # 1. Remove 'params' argument from the function definition
                        node.args.args = [
                            arg for arg in node.args.args if arg.arg != "params"
                        ]
                        
                        # 2. Process the function body to replace parameter access.
                        #    generic_visit will traverse child nodes and trigger 
                        #    visit_Call and visit_Subscript where appropriate.
                        self.generic_visit(node)
                    return node

                def visit_Call(self, node):
                    # This method handles inline calls like: `... = x * params.get("alpha", 0.5)`
                    if (
                        ...
                        # Extract the parameter key (the first argument to .get())
                        if node.args and isinstance(node.args[0], ast.Constant):
                            param_key = node.args[0].value
                            if param_key in best_params:
                                # Replace the entire `params.get(...)` call with a hardcoded constant
                                return ast.Constant(value=best_params[param_key])
                    
                    # Continue visiting child nodes of the call
                    self.generic_visit(node)
                    return node

                def visit_Subscript(self, node):
                    # This method handles subscript access like: `params['beta']`
                    if (
                        isinstance(node.value, ast.Name) and
                        node.value.id == "params" and
                        isinstance(node.slice, ast.Constant)
                    ):
                        param_key = node.slice.value
                        if param_key in best_params:
                            # Replace the subscript access with a hardcoded constant
                            return ast.Constant(value=best_params[param_key])
            ...
            transformer = ParameterInserter()
            tree = transformer.visit(tree)
            ast.fix_missing_locations(tree)

            # Convert modified AST back to code
            optimized_code = ast_to_source(tree)
            
            # Re-apply Numba decorator if necessary
            if self.use_numba:
                optimized_code = add_numba_decorator(optimized_code, target_func_name)

            if self.debug_mode:
                print("Optimized hardcoded code:\n", optimized_code)
            return optimized_code

        except Exception as e:
            if self.debug_mode:
                print(f"[Error] Failed to insert parameters into code using AST: {e}")
                traceback.print_exc()
            # On failure, return the original parameterized code to avoid crashing.
            # This will likely result in default values being used, but prevents a full stop.
            return modified_code
        
    def get_study_name_from_code(self, code_string):
        # Generate unique hash from the code string as the study_name
        return f"study_{hashlib.md5(code_string.encode()).hexdigest()}"

    def get_function_name(self, code_string):
        # Helper to find the main function name (e.g., 'score')
        try:
            tree = ast.parse(code_string)
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    # Prioritize target function name for Bin Packing
                    if node.name == self.target_func_name:
                        return node.name
            # Fallback to the first function found
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    return node.name
        except Exception:
            pass
        return self.target_func_name # Use target_func_name as default fallback

    def optimize_and_evaluate(self, code_string):
        """
        Orchestrates the parameter optimization process.
        """
        if not self.enabled:
            # If disabled (e.g., missing Optuna/LLM), just evaluate the original code robustly
            fitness = self.interface_eval.evaluate(code_string, params=None, n_evals=3)
        ...
        try:
            ...
            # If no parameters extracted or code modification failed, evaluate original
            if not parameters or modified_code == code_string or modified_code is None:
                if self.debug_mode:
                    print("[Info] No parameters extracted or code modification failed. Skipping optimization.")
                fitness = self.interface_eval.evaluate(code_string, params=None, n_evals=3)
                return code_string, fitness if fitness is not None else float("inf")
            
            if self.debug_mode:
                print("Extracted Parameters (Defaults):", parameters)

            # 2. Process parameters: Convert defaults to search ranges (improved handling)
            parameter_ranges = {}
            for key, val in parameters.items():
                if self.debug_mode:
                    print(f"Processing parameter {key} with value: {val} (type: {type(val)})")
                
                # Handle special values like -inf, inf, nan
                if isinstance(val, str):
                    if 'inf' in val.lower():
                        if '-' in val:
                            # -inf case: search around negative values
                            parameter_ranges[key] = (-1000.0, -0.1)
                        else:
                            # +inf case: search around positive values  
                            parameter_ranges[key] = (0.1, 1000.0)
                        if self.debug_mode:
                            print(f"  -> Detected inf value, using range: {parameter_ranges[key]}")
                        continue
                    else:
                        ...
                        try:
                            ...
                            if self.debug_mode:
                                print(f"  -> Cannot convert '{val}' to float, using default 0.0")
                            val = 0.0
                ...
                if isinstance(val, (int, float)):
                    if np.isinf(val):
                        ...
                        parameter_ranges[key] = (-self.range_factor, self.range_factor)
                        if self.debug_mode:
                            print(f"  -> Detected nan value, using range: {parameter_ranges[key]}")
                    elif val == 0.0:
                        # For zero values, use symmetric range around zero
                        parameter_ranges[key] = (-self.range_factor, self.range_factor)
                        if self.debug_mode:
                            print(f"  -> Zero value, using symmetric range: {parameter_ranges[key]}")
                    else:
                        # For normal values, use percentage-based range
                        if abs(val) < 1e-6:  # Very small values
                            parameter_ranges[key] = (val - 0.1, val + 0.1)
                        else:
                            range_size = abs(val) * 0.01
                            parameter_ranges[key] = (val - range_size, val + range_size)
                        if self.debug_mode:
                            print(f"  -> Normal value, using range: {parameter_ranges[key]}")
                else:
                    # Fallback for other types
                    parameter_ranges[key] = (-self.range_factor, self.range_factor)
                    if self.debug_mode:
                        print(f"  -> Unknown type, using default range: {parameter_ranges[key]}")

            if self.debug_mode:
                print("Final Parameter Search Ranges:", parameter_ranges)

            # 3. Define Optuna objective function (with enhanced debugging)
            def objective(trial):
                # Dynamically suggest parameter values
                params = {}
                for key, (low, high) in parameter_ranges.items():
                    params[key] = trial.suggest_float(key, low, high)
                
                if self.debug_mode:
                    print(f"[Debug] Trial {trial.number}: Testing parameters: {params}")
                
                # Create a hardcoded version for this trial to maintain Numba compatibility
                trial_code = self.insert_parameters_into_code(modified_code, params, func_name)
                
                # Save intermediate trial code if code history path is set
                if self.code_history_path is not None:
                    trial_filename = os.path.join(self.code_history_path, f"trial_{trial.number:03d}_params.py")
                    try:
                        os.makedirs(os.path.dirname(trial_filename), exist_ok=True)
                        with open(trial_filename, 'w', encoding='utf-8') as f:
                            # Write header with parameter information
                            f.write(f"# Trial {trial.number} - Parameters: {params}\n")
                            f.write(f"# Generated at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                            f.write(trial_code)
                        if self.debug_mode:
                            print(f"[Debug] Trial {trial.number} code saved to: {trial_filename}")
                    except IOError as e:
                        if self.debug_mode:
                            print(f"[Debug] Error saving trial {trial.number} code: {e}")
                
                # Evaluate the hardcoded version (no params needed, maintains Numba JIT)
                fitness = self.interface_eval.evaluate(trial_code, params=None, n_evals=1)
                
                if self.debug_mode:
                    if fitness is None:
                        print(f"[Debug] Trial {trial.number}: Evaluation failed (None)")
                    else:
                        print(f"[Debug] Trial {trial.number}: Fitness = {fitness}")
                
                if fitness is None:
                    # Prune the trial if evaluation fails
            ...
            try:
                ...
                # Clean up previous study if it exists
                try:
                ...
                if self.debug_mode:
                    print(f"[Warning] RDBStorage initialization failed: {e}. Falling back to InMemoryStorage.")
                storage = InMemoryStorage()


            # Create study (minimization objective)
            ...
            # Silence Optuna internal logging unless in debug mode
            if not self.debug_mode:
                optuna.logging.set_verbosity(optuna.logging.WARNING)

            # n_jobs=1 is recommended when the main EoH loop is already parallelized
            study.optimize(objective, n_trials=self.n_trials, n_jobs=1, gc_after_trial=True)

            # Check results - count only successful (COMPLETE) trials
            successful_trials = [t for t in study.trials if t.state.name == 'COMPLETE']
            if self.debug_mode:
                print(f"[Debug] Optimization completed: {len(study.trials)} total trials, {len(successful_trials)} successful")
                if len(successful_trials) > 0:
                    best_value = study.best_value
                    print(f"[Debug] Best trial value: {best_value}")
                    print(f"[Debug] Best parameters: {study.best_params}")
                    
                    # Show top 3 trials
                    sorted_trials = sorted(successful_trials, key=lambda t: t.value)[:3]
                    print("[Debug] Top 3 trials:")
            ...
            if len(successful_trials) == 0:
                if self.debug_mode:
                    ...
                    print("[Debug] Trial states:", [t.state.name for t in study.trials])
                # Fallback: return original code and re-evaluate
            ...
            if self.debug_mode:
                print(f"[Debug] Inserting best parameters into code: {best_params}")
            optimized_code = self.insert_parameters_into_code(modified_code, best_params, func_name)

            # Save optimized code after parameter search (if code history path is set)
            if self.code_history_path is not None:
                # Create unique filename for this optimization run
                import hashlib
                code_hash = hashlib.md5(code_string.encode()).hexdigest()[:8]
                optimized_filename = os.path.join(self.code_history_path, f"final_optimized_{code_hash}.py")
                # Save the code with optimization summary
                try:
                    os.makedirs(os.path.dirname(optimized_filename), exist_ok=True)
                    with open(optimized_filename, 'w', encoding='utf-8') as f:
                        f.write(f"# Final Optimized Code - Generated at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                        f.write(f"# Best parameters found: {best_params}\n")
                        f.write(f"# Best fitness from optimization: {study.best_value if len(successful_trials) > 0 else 'N/A'}\n")
                        f.write(f"# Total trials: {len(study.trials)}, Successful trials: {len(successful_trials)}\n\n")
                        f.write(optimized_code)
                    if self.debug_mode:
                        print(f"Final optimized code saved to: {optimized_filename}")
                except IOError as e:
                    if self.debug_mode:
                        print(f"Error saving optimized code to {optimized_filename}: {e}")
                
                # Also save a summary of all trials
                summary_filename = os.path.join(self.code_history_path, f"optimization_summary_{code_hash}.txt")
                try:
                    with open(summary_filename, 'w', encoding='utf-8') as f:
                        ...
                        if len(successful_trials) > 0:
                            ...
                            for i, trial in enumerate(sorted_trials):
                                f.write(f"  {i+1}. Trial {trial.number}: Value={trial.value:.6f}, Params={trial.params}\n")
                        else:
                            f.write("No successful trials found.\n")
            ...
            if self.debug_mode:
                print("[Debug] Performing final evaluation of optimized code...")
            final_fitness = self.interface_eval.evaluate(optimized_code, params=None, n_evals=5)
            ...
            # Compare with original fitness (if available from calling context)
            if self.debug_mode:
                print(f"[Debug] Optimization summary:")
                print(f"  - Total trials: {len(study.trials)}")
                print(f"  - Successful trials: {len(successful_trials)}")
                print(f"  - Best fitness from optimization: {study.best_value if len(successful_trials) > 0 else 'N/A'}")
                print(f"  - Final fitness (after re-evaluation): {final_fitness}")

            return optimized_code, final_fitness

        except Exception as e:
            # Handle failures during the optimization process
            if self.debug_mode:
                print(f"[Error] Failed during optimization or evaluation process: {e}")
                traceback.print_exc()
...
#######################################
#######################################
###  Outer layer! Evolution of Prompts (EoH Meta-level)
#######################################
#######################################
class Evolution_Prompt():

    def __init__(self, interface_llm, debug_mode, problem_type, **kwargs):
    ...
    def get_prompt_cross(self,prompts_indivs):
        ##################################################
        ###  Generate prompt for crossover (Meta-level)
        ##################################################
        ...
        # Request a new, different prompt motivated by the existing ones
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def get_prompt_variation(self,prompts_indivs):
        ##################################################
        ###  Generate prompt for variation (Meta-level)
        ##################################################
        
        # Request a new prompt that is a modified version of the provided one
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def initialize(self, prompt_type):
        # Initialize base prompts for crossover and variation
...
class InterfaceEC_Prompt():
    def __init__(self, pop_size, m, interface_llm, debug_mode, select,n_p,timeout, problem_type, **kwargs):

        # Set LLM and EC info
        self.pop_size = pop_size                   # Population size

        self.evol = Evolution_Prompt(interface_llm, debug_mode, problem_type , **kwargs)
        self.m = m                                  # Number of parents for crossover
        self.debug = debug_mode                     # Debug mode
    ...
    def extract_first_quoted_string(self, text):
        # Helper to extract the prompt text from the LLM response
        if text is None:
            return ""
        # Use regex to match the content within the first double quotes
    ...
    def _get_alg(self,pop,operator):
        # Initialize offspring dictionary
        # Objective initialized to 1e9 (poor fitness for minimization)
        offspring = {
            'prompt': None,
            'objective': 1e9, 
        ...
        # Return parents and generated offspring (either single offspring or off_set)
    ...
    def get_algorithm(self, pop, operator):
        # results: List to store generated offspring
        results = []
        try:
            # Generate offspring in parallel
            # Timeout increased (+60s) to account for overhead and LLM latency
            if(operator == 'cross' or operator == 'variation'):
                # Run pop_size times for evolution operators
        ...
        for p, off, off_set in results:
            out_p.append(p)
            if(operator == 'cross' or operator == 'variation'):
                if off and off['prompt']: # Ensure offspring is valid
                    out_off.append(off)
            else:
                # Handle initialization set
                if off_set:
...
#######################################
#######################################
###  Inner layer! Evolution of Heuristics (EoH Base-level)
#######################################
####################################### 

class Evolution():

    def __init__(self, interface_llm, debug_mode,prompts, **kwargs):

        # set prompt interface
        self.prompt_task         = prompts.get_task()         
        self.prompt_func_name    = prompts.get_func_name()    # "score"
        self.prompt_func_inputs  = prompts.get_func_inputs()  # ['item', 'bins']
        self.prompt_func_outputs = prompts.get_func_outputs() # ['scores']
        self.prompt_inout_inf    = prompts.get_inout_inf()    
        self.prompt_other_inf    = prompts.get_other_inf()    
        
        # Format inputs for the prompt string
        if len(self.prompt_func_inputs) > 1:
            self.joined_inputs = ", ".join("'" + s + "'" for s in self.prompt_func_inputs)
        else:
            self.joined_inputs = "'" + self.prompt_func_inputs[0] + "'"

        # Format outputs for the prompt string
    ...
    def get_prompt_initial(self):
        #######################################
        ###  Generate prompt for initial population (Base-level)
        #######################################

        # Construct the prompt content. Added instruction for markdown block.
        prompt_content = self.prompt_task+"\n"\
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations. Provide the code implementation within a ```python ... ``` block."
        return prompt_content

        
    def get_prompt_cross(self,indivs, prompt):
        ##################################################
        ###  Generate prompt for crossover (Base-level)
        ##################################################
        ...
        # Construct the prompt content using the meta-prompt (prompt argument)
        prompt_content = self.prompt_task+"\n"\
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations. Provide the code implementation within a ```python ... ``` block."
    ...
    def get_prompt_variation(self,indiv1, prompt):
        ##################################################
        ###  Generate prompt for variation (Base-level)
        ##################################################
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations. Provide the code implementation within a ```python ... ``` block."
    ...
    def _get_alg(self,prompt_content):
        ...
        def extract(response_text):
            # Extract algorithm description (inside braces)
            alg = re.findall(r"\{(.*)\}", response_text, re.DOTALL)
            
            # Extract code block (inside ```python ... ```)
            code_block = re.findall(r"```python\n(.*?)\n```", response_text, re.DOTALL)
            
            # Fallback extraction patterns if standard formats are missing
            if len(alg) == 0:
                # Look for text before the code block or definitions
                if code_block:
            ...
            if code_block:
                ...
                # Fallback if LLM provided raw code (less reliable)
                c = re.findall(r"(?:import.*?\n)*def\s+\w+\s*\(.*?\):.*", response_text, re.DOTALL)
            
            return alg, c

        algorithm, code = extract(response)

        # Retry mechanism if extraction fails
        n_retry = 1
        MAX_RETRIES = 3
        while (len(algorithm) == 0 or len(code) == 0):
            ...
            # Get new response and retry extraction
            response = self.interface_llm.get_response(prompt_content)
            if response is None:
                 break
    ...
    def initial(self):
        ...
        if self.debug_mode and code_all:
...
#######################################
class InterfaceEC():
    def __init__(self, pop_size, m, interface_llm, debug_mode, interface_prob, select,n_p,timeout,use_numba,**kwargs):

        # Set LLM and EC parameters
        self.pop_size = pop_size                    # Population size
        self.interface_eval = interface_prob        # Evaluator (PROBLEMCONST instance)
        prompts = interface_prob.prompts            # Prompts definitions
        ...
        self.timeout = timeout                      # Timeout definition
        self.use_numba = use_numba                  # Flag to use numba acceleration
    
    # Check if the code already exists in the population (Strict check)
    ...
    def _get_alg(self,pop,operator, prompt):
        ...
        # Crossover (generate dissimilar algorithm)        
        elif operator == "cross":
            parents = self.select.parent_selection(pop,self.m)
            [offspring['code'],offspring['algorithm']] = self.evol.cross(parents, prompt)
        # Variation (generate improved algorithm)
        elif operator == "variation":
            parents = self.select.parent_selection(pop,1)
    ...
    def get_offspring(self, pop, operator, prompt):
        ...
        try:
            ...
            if self.use_numba:
                try:
                    # Fix Numba incompatible code patterns before adding decorator
                    original_code = code
                    code = fix_numba_incompatible_code(code)
                    
                    if self.debug and original_code != code:
                        print("DEBUG: Fixed Numba incompatible code patterns")
                        print("DEBUG: Original code length:", len(original_code))
                        print("DEBUG: Fixed code length:", len(code))
                    ...
                    if self.debug:
                        print(f"Numba decorator addition failed: {e}. Proceeding without Numba.")
            ...
            while self.check_duplicate(pop, code):
                n_retry += 1
                if self.debug:
                    print(f"Duplicated code detected (Retry {n_retry}/{MAX_RETRIES}). Retrying generation...")
                time.sleep(1)
                ...
                # Max retries reached
                if n_retry >= MAX_RETRIES:
                    if self.check_duplicate(pop, code):
                        if self.debug:
                             print("Still duplicated after retry. Skipping this offspring.")
            ...
            # Evaluate fitness directly without nested threading
            # n_evals=1 during evolution for speed
            try:
                fitness = self.interface_eval.evaluate(code, n_evals=1)
                if fitness is not None:
        ...
        # Handle exceptions during generation or evaluation
        except Exception as e:
            if self.debug:
                print(f"Error in get_offspring: {e}")
                # traceback.print_exc()
            # Return invalid offspring structure
            offspring = {
                'algorithm': None,
                'code': None,
                'objective': None,
    ...
    def get_algorithm(self, pop, operator, prompt):
        ...
        out_p = []   # All parents
        out_off = [] # All offspring

        for p, off in results:
            out_p.append(p)
            # Only add valid offspring (where code generation and evaluation succeeded)
    ...
    def population_generation(self):
        # Generate initial population (2 rounds to ensure diversity)
        n_create = 2
        population = []
        # Loop to generate individuals
        for i in range(n_create):
...
#######################################
#######################################
###  Evolution of Heuristics (EOH) Framework Runner
#######################################
#######################################
...
class EOH:
    ...
    def __init__(self, paras, problem, select, manage, **kwargs):
        ...
        self.prob = problem      # Problem definition
        self.select = select     # Parent selection method
        self.manage = manage     # Population management method
        
        # LLM settings (used for initializing LLM interface)
        self.api_endpoint = paras.llm_api_endpoint
        self.api_key = paras.llm_api_key
        self.llm_model = paras.llm_model

        # EoH Prompt Evolution settings
        self.pop_size_cross = 2
        self.pop_size_variation = 2
        self.problem_type = "minimization"

        # Experimental settings       
        self.pop_size = paras.ec_pop_size  # Population size (heuristics)
        ...
        if (paras.ec_m > self.pop_size or paras.ec_m < 2) and self.pop_size >= 2:
            print(f"Warning: ec_m ({paras.ec_m}) should be >= 2 and <= pop_size ({self.pop_size}). Adjusting to m=2.")
            paras.ec_m = 2
        self.m = paras.ec_m                                  # Number of parents (heuristics)
        self.m_prompt = 2                                    # Number of parents (prompts)

        self.debug_mode = paras.exp_debug_mode               

        self.output_path = paras.exp_output_path             

        self.exp_n_proc = paras.exp_n_proc                   # Number of processes
        
        self.timeout = paras.eva_timeout                     # Evaluation timeout

        self.use_numba = paras.eva_numba_decorator           # Numba usage flag
        
        self.paras = paras # Store paras object for access to all settings
    ...
    def add2pop(self, population, offsprings):
        # Iterate over new offsprings
        for off in offsprings:
            # Validation check (InterfaceEC ensures valid objective, but check again)
            if off['objective'] is None:
                continue
                
            # Optional: Duplication check based on objective value (if needed for diversity)
            is_duplicate = False
            if self.debug_mode:
                 for ind in population:
                    if abs(ind['objective'] - off['objective']) < 1e-9:
                        # print("duplicated objective found, skipping addition.")
                        is_duplicate = True
                        break
    ...
    def add2pop_prompt(self, population, offsprings):
         # Iterate over new offsprings
        for off in offsprings:
    ...
    def optimize_best_individual_parameters(self, population):
        """
        Optimize parameters for only the best individual in the population.
        """
        print(f"\n=== Starting Parameter Optimization for Best Individual ===")
        ...
        # Find the best individual (lowest objective value)
        best_individual = min(valid_individuals, key=lambda x: x['objective'])
        print(f"Best individual found with objective: {best_individual['objective']:.5f}")
        
        # Create code_history folder for final generation
        code_history_path = os.path.join(self.output_path, "results/code_history")
        os.makedirs(code_history_path, exist_ok=True)
        
        print(f"Optimizing best individual (Current Obj: {best_individual['objective']:.5f})...")
        
        # Save code before parameter optimization (with numba decorators removed)
        before_filename = os.path.join(code_history_path, f"best_individual_before_optimization.py")
        self.save_code_with_removed_numba(best_individual['code'], before_filename)
        
        # Set code history path for parameter search to save intermediate results
        self.parameter_search.set_code_history_path(code_history_path)
        
        # Run optimization with code history tracking
        optimized_code, final_fitness = self.parameter_search.optimize_and_evaluate(best_individual['code'])

        # Save code after parameter optimization (with numba decorators removed)
        after_filename = os.path.join(code_history_path, f"best_individual_after_optimization.py")
        self.save_code_with_removed_numba(optimized_code, after_filename)

        # Check if optimization improved the result (lower fitness is better)
        if final_fitness < best_individual['objective'] and final_fitness != float("inf"):
            improvement = best_individual['objective'] - final_fitness
            print(f"  -> Optimization SUCCESS. Improvement: {improvement:.5f}. Original: {best_individual['objective']:.5f} -> New: {final_fitness:.5f}")
            
            # Create updated best individual
            optimized_individual = {
                'algorithm': best_individual['algorithm'] + " [Optimized]",
                'code': optimized_code,
                'objective': np.round(final_fitness, 5),
                'other_inf': "Parameters Optimized - Best Individual"
            }
            ...
            for ind in population:
                if ind == best_individual:
                    new_population.append(optimized_individual)
                else:
            ...
            if self.debug_mode:
                if final_fitness == float('inf'):
                    print(f"  -> Optimization FAILED (infinite fitness). Keeping original (Obj: {best_individual['objective']:.5f})")
                else:
                    difference = final_fitness - best_individual['objective']
                    print(f"  -> Optimization finished, but no improvement found. Original: {best_individual['objective']:.5f}, New: {final_fitness:.5f} (diff: +{difference:.5f}). Keeping original.")
            else:
                print(f"  -> Optimization finished, but no improvement found (New Obj: {final_fitness:.5f}). Keeping original.")
        ...
        print("=== Parameter Optimization for Best Individual Finished ===")
    ...
    def run(self):
        ...
        # Setup problem evaluation interface
        interface_prob = self.prob
        
        # Initialize Prompt Evolution Interfaces (EoH Meta-Level)
        # Pass the central LLM interface instance
        ...
        # Initialize Heuristic Evolution Interface (EoH Base-Level)
        ...
        if not population:
             print("Error: Initial population generation failed. Cannot proceed.")
             return

        # Apply population management (sorting and truncation)
        population = self.manage.population_management(population, self.pop_size)
        # --------------------------------------------------------------

        print(f"Pop initial (after optimization): ")
        # Print initial objectives (fitness is relative gap, lower is better)
        for off in population:
            print(f" Obj: {off['objective']:.5f}", end="|")
        print()
        print("Initial population created and optimized!")
        ...
        # Variables for adaptive prompt evolution strategy
        worst_objectives = [] # Track worst objective in the population over generations
        delay_turn = 2        # Window size for checking stagnation
        change_flag = 0       # Flag/Cooldown for prompt evolution phase
        last_prompt_evolution_gen = -1 # Last generation where prompt evolution occurred
        max_k = 4             # Max history size for prompt performance tracking (Heap size)

        for gen in range(n_start, self.n_pop):  
            
            # --- Adaptive Prompt Evolution Logic (Meta-Level) ---
            
            # If change_flag is active (cooldown), finalize the prompt population update.
            if(change_flag):
                change_flag -= 1
                if(change_flag == 0):
                    print("--- Finalizing Prompt Population Update (Sorting/Selection) ---")
                    # Apply management based on the gathered performance data
            ...
            # Check for stagnation in the heuristic population (based on worst individual)
            if(len(worst_objectives) >= delay_turn and worst_objectives[-1] >= worst_objectives[-delay_turn] - 1e-9 and gen - last_prompt_evolution_gen > delay_turn):
                ...
                # Reset objectives for newly added prompts (identified by 1e9)
                for prompt in cross_operators:
                ...
                # Reset objectives for newly added prompts
                for prompt in variation_operators:
                     if prompt["objective"] == 1e9:
                        prompt["number"] = []
            ...
            # 1. Crossover operations using evolved cross prompts
            for i in range(len(cross_operators)):
                ...
                for off in offsprings:
                    # Offsprings returned by get_algorithm have valid objectives
                    print(f"{off['objective']:.5f}", end="|")

                    # Track performance using a max-heap (storing negative objectives for minimization)
                    # Tracks the top 'max_k' performances generated by this prompt
            ...
            # 2. Variation operations using evolved variation prompts
            for i in range(len(variation_operators)):
                ...
                for off in offsprings:
                    ...
                    # Track performance using a max-heap
            ...
            # --- Parameter Optimization Phase (Integrated Step) ---
            # Only optimize parameters for the best individual in the last generation
            if gen == self.n_pop - 1:
                print(f"\n=== Final Generation: Starting Parameter Optimization for Best Individual ===")
                population = self.optimize_best_individual_parameters(population)
            else:
                print(f"\n=== Generation {gen + 1}: Skipping parameter optimization (not final generation) ===")
            ...
            try:
                ...
                print(f"Error saving population at generation {gen+1}: {e}")
            ...
            if population:
                try:
                    ...
                    print(f"Error saving best individual at generation {gen+1}: {e}")
            ...
            for i in range(len(population)):
                ...
                print(f"{obj:.5f} ", end="")
                if obj < current_worst:
                    current_worst = obj

            # Update stagnation tracking (using the worst objective in the sorted population)
            if population:
...
class Methods():
    # Set parent selection method and population management method
    def __init__(self,paras,problem) -> None:
        self.paras = paras      
        self.problem = problem
        
        # Selection methods (using imported functions directly as requested)
        ...
        # Management methods (using imported functions directly as requested)
...
class EVOL:
    ...
    def __init__(self, paras, prob=None, **kwargs):
        ...
        print("------------------------------------------------------")
        print("---  Start EoH + Automatic Parameter Optimization  ---")
        print("------------------------------------------------------")
        # Create folder #
...
if __name__ == "__main__":
    ...
    paras.set_paras(method = "eoh",    # ['ael','eoh']
                    problem = "bp_online", # Set to bp_online
    ...
    print("Code merged successfully. Execution block is commented out.")